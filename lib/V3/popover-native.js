...
var Popover = function( element, options ) {
  ...
  if (containerElement) {
    ...
    if (containerDataElement) {
      this[container] = containerDataElement;
    } else {
      this[container] = navbarFixedTop ? navbarFixedTop
  ...
  var popover = null, timer = 0, placementSetting = this[placement],
    ...
    dismissHandlerToggle = function(type){
      if ('click' == self[trigger] || 'focus' == self[trigger]) {
  ...
  if ( !(stringPopover in element) ) { // prevent adding event handlers twice
    ...
    } else if ('click' == self[trigger] || 'focus' == self[trigger]) {
