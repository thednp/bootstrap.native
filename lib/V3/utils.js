...
var globalObject = typeof global !== 'undefined' ? global : this||window,
  ...
  styleTip = function(link,element,position,parent) { // both popovers and tooltips (target,tooltip,placement,elementToAppendTo)
    ...
    if ( position === left || position === right ) { // secondary|side positions
      ...
      if ( position === top) { // TOP
        topPosition =  rect[top] + scroll.y - elementDimensions.h; // - (rightExceed && leftExceed ? arrow[offsetHeight] : 0) // some strange bug with all browsers, hack only works with tooltips
      ...
      if (halfLeftExceed) {
        ...
        leftPosition = rect[left] + scroll.x - elementDimensions.w/2 + linkDimensions.w/2; // + (rightExceed && leftExceed ? arrow[offsetWidth] : 0) // some strange bug with all browsers
      }
    }
